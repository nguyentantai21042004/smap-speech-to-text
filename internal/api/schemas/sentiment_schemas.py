...
class SentimentRequest(BaseModel):
    ...
    class Config:
        json_schema_extra = {
            "example": {"text": "Sản phẩm này rất tuyệt vời, tôi rất hài lòng!"}
        }
...
class SentimentResult(BaseModel):
    ...
    probabilities: Dict[str, float] = Field(
        ..., description="Probability distribution over all labels"
    )
    segmented_text: Optional[str] = Field(
        None, description="Segmented text (if segmentation enabled)"
    )
...
class SentimentResponse(BaseModel):
    ...
    data: Optional[SentimentResult] = Field(
...
class BatchSentimentResponse(BaseModel):
    ...
    success: bool = Field(..., description="Whether the request succeeded")
    data: Optional[List[SentimentResult]] = Field(
        None, description="List of sentiment results"
    )
    error: Optional[str] = Field(None, description="Error message if failed")
...
class ModelInfoResponse(BaseModel):
    ...
    segmentation_enabled: bool = Field(
        ..., description="Whether segmentation is enabled"
    )
