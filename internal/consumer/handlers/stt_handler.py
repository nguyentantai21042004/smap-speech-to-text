...
async def handle_stt_message(message: AbstractIncomingMessage) -> None:
    ...
    try:
        ...
        try:
            ...
            logger.warning(f"Message rejected (invalid JSON)")
            return
        ...
        if not job_id:
            ...
            await message.reject(requeue=False)
            logger.warning(f"Message rejected (missing job_id)")
            return
        ...
        error_formatted = format_exception_short(e, "Transient error")
        logger.error(f"{error_formatted}")

        # Reject and requeue message for retry
        await message.reject(requeue=True)
        logger.warning(f"HANDLER: Message requeued for retry: job_id={job_id}")

    except PermanentError as e:
        # Permanent errors - don't requeue
        logger.error(f"HANDLER: Permanent error for job {job_id}: {e}")
        error_formatted = format_exception_short(e, "Permanent error")
        logger.error(f"{error_formatted}")
        ...
        error_formatted = format_exception_short(e, "Handler error")
        logger.error(f"{error_formatted}")

        # Reject and requeue for retry (may be temporary issue)
        await message.reject(requeue=True)
        logger.warning(
            f"HANDLER: Message requeued due to unexpected error: job_id={job_id}"
        )
