...
def validate_dependencies() -> None:
    ...
    if whisper_path.exists() and whisper_path.is_file():
        ...
        for alt_path in alternative_paths:
            alt_resolved = alt_path.resolve() if not alt_path.is_absolute() else alt_path
            if alt_resolved.exists() and alt_resolved.is_file() and os.access(alt_resolved, os.X_OK):
